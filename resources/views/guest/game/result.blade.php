...
<div class="w-128 mx-auto">
    <div class="flex flex-col justify-center items-center bg-zinc-100 mt-5 rounded-2xl border-2 border-sky-100 p-5">
        ...
        <p class="mt-3 capitalize">game id: {{ $game->id}}</p>
        <div id="bg" class="grid grid-cols-2 items-center capitalize mt-5 gap-8 text-2xl bg-zinc-200 p-5 rounded-2xl border border-zinc-300 text-white">
            <p>player</p>
            <p>{{ $game->user->name}}</p>
            <p>bot</p>
            ...
            <p class="flex items-center gap-2">{{$game->result}}</p>
                <input id="game_winner" type="hidden" value="{{$game->result}}">
            <p>credit+</p>
...
<script src="{{asset('js/game-result.js')}}"></script>
@endsection